...
**Raíz**: el client `woocommerce` en `tpv7.2465_api_clients` no tenía el scope `stock:read`, pero el router `GET /products/{id}/stock` lo exige (`classes/Router.php:267` — `$this->auth->requireAuth('stock:read')`).
...
| # | Dónde | Qué | Línea |
|---|-------|-----|-------|
| 1 | `tpv7.2465_api_clients.scopes` (BD API) | Añadido `stock:read` | 1 UPDATE |
...
- `/var/www/html/tpv85/.../.memoria_wp/tests/bugs_focused.sh` — tests TDD que reproducen solo BUG-WC-001 y BUG-WC-003. Antes del fix: 3/6 PASS (los 2 bugs fallan). Después: 6/6 PASS.
